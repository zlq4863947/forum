...
$(function() {
	...
	// 業務フローパターンメンテナンス情報を初期化する
	initDivisionInfo();
});

//インバリッドのスタイルを削除する
function removeValidate(){
	$("#flowPatternCd").nextAll().removeClass("textbox-invalid");
	$("#flowPatternCd").next().children().removeClass("validatebox-invalid");
}
...
function initDivisionInfo() {
	...
	if(!id || id=="null") {
		...
		$("#flowPatternCd").textbox("setValue",id)
		// 新規登録ボタンの表示名に「変更」を変更する
		$("#btnRegister .l-btn-text").text("変更");
		$("#btnRegister").attr("onClick","update()");
		// 親画面から渡された部店コードが存在しない場合、エラーとする
		if(!$("#flowPatternCd").textbox("getValue") || $("#flowPatternCd").textbox("getValue")=="null") {
			$("#p_errorMessage")[0].innerHTML = getMessage("E2090");
			$("#btnRegister").linkbutton('disable');
			return;
		} else {
			// 業務フローパターンメンテナンス情報を取得する
			var setting = {
				data : {"flowPatternCd":$("#flowPatternCd").val()},
				url : "/IDMS0646/getFlowPatternInfo.htm",
				hasLoading : true,
				hasContentType : false,
				success : function(res) {
					// 貰ったデータを画面の項目に設定する
					$("#IDMS0646Form").form('load',res);
					// 下記の項目が編集不可にする
					// 部店コード
		  			$('#flowPatternCd').textbox('disable');
				}
			};
			postAjax(setting);
		}
...
//業務フローパターンメンテナンス情報を新規登録する
function register() {
	removeValidate();
	$("#p_errorMessage")[0].innerHTML = '&nbsp;';
	removeAllErrorForEasyUI("IDMS0646Form");
	// 画面の検証
	if($('#IDMS0646Form').form('validate') ==false){
...
// 業務フローパターンメンテナンス情報新規登録のコールバック関数
function exceRegister() {
	var setting = {
		data : $("#IDMS0646Form").serialize(),
		url : "/IDMS0646/register.htm",
		hasLoading : true,
		success : function(res) {
			if(!res.result) { // 実行失敗の場合
				...
				if(res.errorCode == "E2085") { // 部署名の重複エラー
					// 背景色に赤色を設定する
					$("#flowPatternCd").nextAll().addClass("textbox-invalid");
					$("#flowPatternCd").next().children().addClass("validatebox-invalid");
				}
			} else { // 実行成功の場合
				$.messager.show({
					title:'情報',
...
//業務フローパターンメンテナンス情報変更のコールバック関数
function exceUpdate() {
	// 「部署名」を活性化にする
	$("#flowPatternCd").textbox("enable");
	var setting = {
		data : $("#IDMS0646Form").serialize(),
		url : "/IDMS0646/update.htm",
		hasLoading : true,
		success : function(res) {
			if(!res.result) { // 実行失敗の場合
				$("#p_errorMessage")[0].innerHTML = res.errorMessage;

...
function initDivisionInfo() {
	...
	if(!id || id=="null") {
		...
		if(!$("#divisionName").textbox("getValue") || $("#divisionName").textbox("getValue")=="null") {
			...
			// STAR扱者コードメンテナンス情報を取得する
			var setting = {
				data : {"divisionName":$("#divisionName").val()},
				url : "/IDMS0645/getDivisionInfo.htm",
				hasLoading : true,
				hasContentType : false,
				success : function(res) {
					// 貰ったデータを画面の項目に設定する
					$("#IDMS0645Form").form('load',res);
					// 下記の項目が編集不可にする
					// 部店コード
...
//STAR扱者コードメンテナンス情報を新規登録する
function register() {
	removeValidate();
	$("#p_errorMessage")[0].innerHTML = '&nbsp;';
	removeAllErrorForEasyUI("IDMS0645Form");
	// 画面の検証
	if($('#IDMS0645Form').form('validate') ==false){
...
// STAR扱者コードメンテナンス情報新規登録のコールバック関数
function exceRegister() {
	var setting = {
		data : $("#IDMS0645Form").serialize(),
		url : "/IDMS0645/register.htm",
		hasLoading : true,
		success : function(res) {
...
//STAR扱者コードメンテナンス情報変更のコールバック関数
function exceUpdate() {
	// 「部署名」を活性化にする
	$("#divisionName").textbox("enable");
	var setting = {
		data : $("#IDMS0645Form").serialize(),
		url : "/IDMS0645/update.htm",
		hasLoading : true,
		success : function(res) {
			if(!res.result) { // 実行失敗の場合
				$("#p_errorMessage")[0].innerHTML = res.errorMessage;

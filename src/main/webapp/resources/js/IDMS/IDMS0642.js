...
$(function() {
	...
	// システムカテゴリメンテナンス情報を初期化する
	initSystemAuthorityInfo();
});

// インバリッドのスタイルを削除する
function removeValidate(){
	$("#categoryCd").nextAll().removeClass("textbox-invalid");
	$("#categoryCd").next().children().removeClass("validatebox-invalid");
	$("#categoryName").nextAll().removeClass("textbox-invalid");
	$("#categoryName").next().children().removeClass("validatebox-invalid");
}
...
function initSystemAuthorityInfo() {
	...
	if(!callData.systemCd || callData.systemCd=="null") {
		...
		$("#categoryCd").textbox("setValue",callData.categoryCd);
		...
		if(!$("#systemCd").textbox("getValue") || $("#systemCd").textbox("getValue")=="null") {
			...
			// システムカテゴリメンテナンス情報を取得する
			var setting = {
				data : {
					"systemCd":$("#systemCd").val(),
					"categoryCd":$("#categoryCd").val()},
				url : "/IDMS0642/getSystemCategoryInfo.htm",
				hasLoading : true,
				hasContentType : false,
				success : function(res) {
					// 貰ったデータを画面の項目に設定する
					$("#IDMS0642Form").form('load',res);
					// 下記の項目が編集不可にする
					// システムコード
		  			$("#systemCd").textbox("disable");
					// システムカテゴリコード
		  			$("#categoryCd").textbox("disable");
				}
			};
			postAjax(setting);
		}
...
// システムカテゴリメンテナンス情報を新規登録する
function register() {
	removeValidate();
	$("#p_errorMessage")[0].innerHTML = "&nbsp;";
	removeAllErrorForEasyUI("IDMS0642Form");
	// 画面の検証
	if($('#IDMS0642Form').form('validate') ==false){
...
// システムカテゴリメンテナンス情報新規登録のコールバック関数
function exceRegister() {
	var setting = {
		data : $("#IDMS0642Form").serialize() + "&systemName=" + $("#systemCd").textbox("getText"),
		url : "/IDMS0642/register.htm",
		hasLoading : true,
		success : function(res) {
			if(!res.result) { // 実行失敗の場合
				...
				if(res.errorCode == "E2093") {
					// 背景色に赤色を設定する
					$("#categoryCd").nextAll().addClass("textbox-invalid");
					$("#categoryCd").next().children().addClass("validatebox-invalid");
				} else if(res.errorCode == "E2082") { // カテゴリ名称の重複エラー
					// 背景色に赤色を設定する
					$("#categoryName").nextAll().addClass("textbox-invalid");
					$("#categoryName").next().children().addClass("validatebox-invalid");
				}
			} else { // 実行成功の場合
				$.messager.show({
					title:"情報",
...
// システムカテゴリメンテナンス情報変更のコールバック関数
function exceUpdate() {
	// 「システムコード」を活性化にする
	$("#systemCd").textbox("enable");
	// 「カテゴリコード」を活性化にする
	$("#categoryCd").textbox("enable");
	var setting = {
		data : $("#IDMS0642Form").serialize() + "&systemName=" + $("#systemCd").textbox("getText"),
		url : "/IDMS0642/update.htm",
		hasLoading : true,
		success : function(res) {
			if(!res.result) { // 実行失敗の場合
				$("#p_errorMessage")[0].innerHTML = res.errorMessage;

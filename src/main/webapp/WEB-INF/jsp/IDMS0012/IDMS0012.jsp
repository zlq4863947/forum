...
var oldOrganizationFlag;
var organizationRank ;
function userInfo (){
	organizationCode = "<%=request.getParameter("organizationCode")%>";
	organizationName = "<%=request.getParameter("organizationName")%>";
	oldOrganizationFlag = "<%=request.getParameter("oldOrganizationFlag")%>";
	organizationRank = "<%=request.getParameter("organizationRank")%>";
}
</script>
<script type="text/javascript" src="<%=request.getContextPath()%>/resources/js/IDMS/IDMS0012.js?${initParam.version}"></script>
</head>
<body style="text-align:center;padding:10px;">
	...
	<a onclick="btnSearch()" class="easyui-linkbutton" id="btnSearchOrganization" style="width: 90px; height: 25px;">検索</a>
	<br><br>
	</div>
